...
# ── URDF joint definitions for direct TF publishing ──────────
# (parent, child, xyz, rpy, joint_name_or_None, axis_sign)
# axis_sign: 0=fixed, +1=revolute axis Z, -1=revolute axis -Z
FIXED_JOINTS = [
...
class TeleopAll(Node):
    ...
    def _init_rviz(self):
        ...
        static_tfs = []
        for parent, child, xyz, rpy in FIXED_JOINTS:
    ...
    def publish_rviz_state(self, linear: float, angular: float):
        ...
        v_left = (linear - angular * WHEEL_SEPARATION / 2.0) / WHEEL_RADIUS
        v_right = (linear + angular * WHEEL_SEPARATION / 2.0) / WHEEL_RADIUS
        ...
        for parent, child, xyz, rpy, jname, axis_sign in DYNAMIC_JOINTS:
            angle = self.joint_positions.get(jname, 0.0)
            origin_q = quat_from_rpy(rpy[0], rpy[1], rpy[2])
            joint_q = (0.0, 0.0,
                       math.sin(axis_sign * angle / 2.0),
                       math.cos(axis_sign * angle / 2.0))
